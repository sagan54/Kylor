...
// ─── Image Card ───────────────────────────────────────────────────────────────
function ImageCard({ item, view, onDelete, onOpen, onToggleFavorite, onDownload, onShare }) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        position: "relative",
        background: gradient,
        aspectRatio: (item.ratio || "1:1").replace(":", "/"),
            ...
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button
                ...
                style={{
                  ...
                  color: item.starred ? "#fbbf24" : "white",
                  display: "grid",
                  placeItems: "center",
                  cursor: "pointer",
                }}
  ...
  return (
    <div style={{ display: "grid", gap: "8px" }}>
      ...
      >
        ...
        <div style={{ fontSize: "12px", color: files.length > 0 ? "#a78bfa" : C.textDim, fontWeight: 600 }}>
          {files.length}/10
        </div>
      </motion.div>
...
export default function ImagePage() {
  ...
  useEffect(() => {
    const saved = safeJsonParse(localStorage.getItem("kylor_image_generations"), []);
    if (Array.isArray(saved) && saved.length) {
      setRecentItems(saved);
      setFeaturedId(saved[0]?.id ?? null);
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("kylor_image_generations", JSON.stringify(recentItems));
  }, [recentItems]);
  ...
  }, [recentItems, favoritesOnly, contentFilter]);

  const featured = useMemo(() => {
    return filteredItems.find((x) => x.id === featuredId) || filteredItems[0] || null;
  }, [filteredItems, featuredId]);

  const history = useMemo(() => {
    if (!featured) return [];
    return filteredItems.filter((x) => x.id !== featured.id);
  }, [filteredItems, featured]);

  function updateItem(id, updater) {
    setRecentItems((prev) => prev.map((item) => (item.id === id ? updater(item) : item)));
  }

  function toggleFavorite(id) {
    updateItem(id, (item) => ({ ...item, starred: !item.starred }));
  }
  ...
  function clearAllOutputs() {
    ...
    setFeaturedId(null);
    setLightboxItem(null);
    localStorage.removeItem("kylor_image_generations");
  }

  async function handleDownload(item) {
    if (!item?.url) return;
    const filename = `kylor-${item.id}.png`;
    await downloadImage(item.url, filename);
    setViewerMessage("Downloaded");
  }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (images.length) {
        ...
        setRecentItems((prev) => [...newItems.reverse(), ...prev]);
        setFeaturedId(newItems[0].id);
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <span style={{ fontSize: "11px", color: prompt.length > charLimit * 0.9 ? "#f87171" : C.textDim }}>
              ...
              {generating ? (
                <div style={{ height: "100%", display: "grid", placeItems: "center" }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <p style={{ margin: "0 0 6px", color: C.text, fontSize: "16px", fontWeight: 700 }}>
                ...
                <div style={{ height: "100%", display: "grid", placeItems: "center" }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <p style={{ margin: "0 0 4px", color: C.text, fontSize: "15px", fontWeight: 600 }}>
                      Video Generation
                    </p>
                    <p style={{ margin: 0, color: C.textMuted, fontSize: "13px" }}>
                      Coming soon — video outputs will appear here.
                    </p>
                  </div>
                </div>
              ) : contentFilter === "Audio" ? (
                <div style={{ height: "100%", display: "grid", placeItems: "center" }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <p style={{ margin: 0, color: C.textMuted, fontSize: "13px" }}>
                      Coming soon — audio outputs will appear here.
                    </p>
                  </div>
                </div>
                ...
                <div style={{ height: "100%", display: "grid", placeItems: "center" }}>
                  ...
                  >
                    ...
                    <p style={{ margin: "0 0 8px", color: C.text, fontSize: "16px", fontWeight: 700 }}>
                      Your canvas is empty
                    </p>
                    <p style={{ margin: "0 0 20px", color: C.textMuted, fontSize: "13px", lineHeight: 1.7 }}>
                      Describe what you want to create and hit Generate — your image will appear here.
                    </p>
                    <motion.button
                      whileTap={{ scale: 0.96 }}
                ...
                <div
                  style={{
                    padding: "16px",
                    display: "grid",
                    gridTemplateRows: "minmax(540px, 72vh) auto",
                    gap: "16px",
                    minHeight: "100%",
                  }}
                >
                  ...
                  >
                      ...
                      {featured?.url ? (
                        ...
                        <div
                          style={{
                            ...
                            display: "grid",
                            placeItems: "center",
                            background: CARD_GRADIENTS[(featured?.id || 1) % CARD_GRADIENTS.length],
                          }}
                        >
                          <ImageIcon size={46} color="rgba(255,255,255,0.2)" />
                        </div>
                      )}
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: "rgba(8,10,16,0.85)",
                            backdropFilter: "blur(10px)",
                            color: featured?.starred ? "#fbbf24" : "white",
                            display: "grid",
                            placeItems: "center",
                            cursor: "pointer",
                          }}
                        >
                          <Star size={16} fill={featured?.starred ? "#fbbf24" : "none"} />
                        </button>
                    ...
                    >
                      ...
                      >
                        <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", minWidth: 0 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: "12px", color: C.text, fontWeight: 700 }}>Kylor V1</div>
                            <div style={{ fontSize: "11px", color: C.textMuted }}>Generated image</div>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            border: `1px solid ${C.border}`,
                            background: C.surface,
                            color: featured?.starred ? "#fbbf24" : C.textMuted,
                            display: "grid",
                            placeItems: "center",
                            cursor: "pointer",
                          }}
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                        {[featured?.ratio, featured?.mode || mode, featured?.style || "Custom"].map((item) => (
                          <div
                            key={item}
                            style={{
                              height: "28px",
                      ...
                      >
                        <div style={{ fontSize: "11px", color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600 }}>
                          Details
                        </div>
                        <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.8)" }}>
                          Created: {featured?.createdAt ? new Date(featured.createdAt).toLocaleString() : "—"}
                        </div>
                        <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.8)" }}>
                          Outputs stored: {recentItems.length}
                        </div>
                      </div>
                  ...
                  >
                    ...
                    >
                      <div style={{ fontSize: "12px", color: C.textMuted, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>
                        Older Generations · {history.length}
                      </div>
                    </div>

                    {history.length === 0 ? (
                      <div style={{ fontSize: "13px", color: C.textMuted }}>No older generations yet.</div>
                    ) : assetView === "grid" ? (
                      <div
                        style={{
                          display: "grid",
                      ...
                      <div style={{ display: "grid", gap: "4px", maxHeight: "520px", overflowY: "auto", paddingRight: "2px" }}>
                        {history.map((item) => (
                          <ImageCard
                            key={item.id}
                            item={item}
        ...
        {lightboxItem && (
          ...
          >
            ...
            >
              {lightboxItem.url ? (
                ...
                <div
                  style={{
                    width: "600px",
                    height: "700px",
                    background: CARD_GRADIENTS[lightboxItem.id % CARD_GRADIENTS.length],
                    display: "grid",
                    placeItems: "center",
                  }}
                >
              ...
              >
                <p style={{ margin: "0 0 4px", fontSize: "15px", fontWeight: 600, color: "white" }}>
                  {lightboxItem.prompt}
                </p>
                <p style={{ margin: 0, fontSize: "12px", color: C.textMuted }}>
                  {lightboxItem.ratio} · {lightboxItem.mode || mode}
                  {lightboxItem.style ? ` · ${lightboxItem.style}` : ""}
                </p>
              </div>
            </motion.div>
          </motion.div>
