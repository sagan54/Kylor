...
function ImageCard({ item, view }) {
  ...
  if (view === "list") {
    return (
      ...
      >
        <div style={{ width: "48px", height: "48px", borderRadius: radius.sm, background: gradients[item.id % gradients.length], flexShrink: 0, border: `1px solid ${C.border}` }} />
          ...
          {hovered && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: "flex", gap: "6px" }}>
              {[Download, Share2, Trash2].map((Icon, i) => (
                <button key={i} style={{ width: "30px", height: "30px", borderRadius: "8px", border: `1px solid ${C.border}`, background: C.surface, color: C.textMuted, display: "grid", placeItems: "center", cursor: "pointer" }}><Icon size={13} /></button>
              ))}
            </motion.div>
          )}
        </AnimatePresence>
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      onHoverStart={() => setHovered(true)} onHoverEnd={() => setHovered(false)}
      style={{ borderRadius: radius.lg, border: `1px solid ${hovered ? C.borderHover : C.border}`, overflow: "hidden", cursor: "pointer", position: "relative", background: gradients[item.id % gradients.length], aspectRatio: item.ratio.replace(":", "/"), transition: "border-color 0.16s ease" }}
    >
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "6px" }}>
                {[Download, Share2].map((Icon, i) => (
                  <button key={i} style={{ height: "26px", padding: "0 10px", borderRadius: "7px", border: `1px solid ${C.border}`, background: "rgba(0,0,0,0.5)", color: "white", display: "inline-flex", alignItems: "center", cursor: "pointer", fontSize: "11px", fontFamily: "inherit" }}><Icon size={11} /></button>
                ))}
              </div>
            </div>
          </motion.div>
...
function DropZone({ onFiles }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ width: "34px", height: "34px", borderRadius: radius.sm, flexShrink: 0, background: C.accentSoft, border: `1px solid ${C.accentBorder}`, display: "grid", placeItems: "center" }}><Upload size={14} color="#a78bfa" /></div>
      <div>
...
export default function ImagePage() {
  ...
  const handleGenerate = () => {
    ...
    setGenerating(true); setShowRecent(false);
    ...
    <main style={{
      ...
      <div style={{ display: "grid", gridTemplateRows: "48px 1fr", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", height: "100%", overflow: "hidden" }}>
          ...
          {/* ── Left Panel ── */}
          <div style={{ borderRight: `1px solid ${C.border}`, background: "linear-gradient(180deg, rgba(7,9,15,0.98), rgba(9,11,17,0.98))", padding: "16px", display: "grid", gridTemplateRows: "auto auto auto 1fr auto", gap: "12px", overflow: "hidden", height: "100%" }}>
            ...
            <motion.button whileHover={{ borderColor: C.accentBorder }} whileTap={{ scale: 0.99 }}
              style={{ width: "100%", padding: "10px 12px", borderRadius: radius.md, border: `1px solid ${C.border}`, background: C.surface, color: C.text, display: "flex", alignItems: "center", gap: "10px", cursor: "pointer", fontFamily: "inherit", transition: "all 0.16s ease" }}
            >
              ...
              <div style={{ flex: 1, minWidth: 0, textAlign: "left" }}>
                <div style={{ fontSize: "14px", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Kylor V1</div>
            ...
            {/* Prompt */}
            <div style={{ borderRadius: radius.md, border: `1px solid ${C.border}`, background: C.surface, padding: "12px", display: "grid", gridTemplateRows: "1fr auto", minHeight: 0 }}>
              <textarea
                ref={textareaRef} value={prompt} onChange={(e) => setPrompt(e.target.value.slice(0, charLimit))}
                placeholder="Describe the image you want to create..."
                style={{ width: "100%", border: "none", background: "transparent", color: C.text, resize: "none", fontFamily: "inherit", fontSize: "13.5px", lineHeight: 1.7, outline: "none", height: "100%", minHeight: 0, boxSizing: "border-box" }}
              />
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "10px", paddingTop: "10px", borderTop: `1px solid ${C.border}` }}>
                <div style={{ display: "flex", gap: "6px" }}>
                  <motion.button whileTap={{ scale: 0.95 }} style={{ height: "30px", padding: "0 12px", borderRadius: "9px", border: `1px solid ${C.accentBorder}`, background: C.accentSoft, color: "#a78bfa", fontSize: "12px", fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Styles</motion.button>
                  <motion.button whileTap={{ scale: 0.95 }} style={{ height: "30px", padding: "0 12px", borderRadius: "9px", border: `1px solid ${C.border}`, background: C.surface, color: C.textMuted, fontSize: "12px", cursor: "pointer", fontFamily: "inherit" }}>Negative</motion.button>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <span style={{ fontSize: "11px", color: charCount > charLimit * 0.9 ? "#f87171" : C.textDim }}>{charCount}/{charLimit}</span>
                  <motion.button whileTap={{ scale: 0.92 }} style={{ width: "32px", height: "32px", borderRadius: "10px", border: `1px solid ${C.accentBorder}`, background: C.accentSoft, color: "#a78bfa", display: "grid", placeItems: "center", cursor: "pointer" }}>
                    <Sparkles size={14} />
                  </motion.button>
                </div>
              </div>
            ...
            <div ref={panelRef} style={{ position: "relative" }}>
              <AnimatePresence>
                {settingsOpen && (
                  <motion.div initial={{ opacity: 0, y: 16, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.97 }} transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}
                    style={{ position: "absolute", left: 0, right: 0, bottom: "58px", zIndex: 10, borderRadius: "18px", border: `1px solid ${C.border}`, background: "rgba(14,16,26,0.99)", backdropFilter: "blur(16px)", boxShadow: "0 28px 80px rgba(0,0,0,0.5)", padding: "16px", display: "grid", gap: "16px" }}
                  >
                    <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "10px" }}>
                <motion.button whileHover={{ borderColor: C.borderHover }} whileTap={{ scale: 0.97 }} onClick={() => setSettingsOpen((p) => !p)}
                  style={{ height: "46px", padding: "0 14px", borderRadius: radius.md, border: `1px solid ${C.border}`, background: "#0d0f18", color: "rgba(255,255,255,0.85)", display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "13px", fontFamily: "inherit", whiteSpace: "nowrap", transition: "border-color 0.15s ease" }}
                >
                  <Settings size={14} />
                ...
                <motion.button whileHover={{ boxShadow: "0 18px 40px rgba(124,58,237,0.38)" }} whileTap={{ scale: 0.98 }} onClick={handleGenerate}
                  style={{ height: "46px", borderRadius: radius.md, border: "none", background: prompt.trim() ? "linear-gradient(135deg, #4f46e5, #7c3aed)" : "rgba(255,255,255,0.06)", color: prompt.trim() ? "white" : C.textMuted, cursor: prompt.trim() ? "pointer" : "default", fontSize: "14px", fontWeight: 700, boxShadow: prompt.trim() ? "0 14px 32px rgba(124,58,237,0.28)" : "none", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "8px", fontFamily: "inherit", transition: "all 0.2s ease" }}
                >
          ...
          <div style={{ background: "rgba(4,5,12,0.95)", display: "grid", gridTemplateRows: `${notifDismissed ? "0px" : "44px"} 48px 1fr`, height: "100%", overflow: "hidden", transition: "grid-template-rows 0.24s ease" }}>

            {/* Notification */}
            <AnimatePresence>
              {!notifDismissed && (
                <motion.div exit={{ opacity: 0, height: 0 }}
                  style={{ borderBottom: `1px solid ${C.border}`, background: "#12141e", padding: "0 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "10px", overflow: "hidden" }}
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", color: "rgba(255,255,255,0.75)", fontSize: "13px" }}>
                    ...
                    <button style={{ border: "none", background: "transparent", color: "#a78bfa", cursor: "pointer", fontWeight: 700, fontSize: "13px", fontFamily: "inherit" }}>Allow</button>
                  </div>
                  <motion.button whileTap={{ scale: 0.9 }} onClick={() => setNotifDismissed(true)} style={{ border: "none", background: "transparent", color: C.textMuted, cursor: "pointer", display: "grid", placeItems: "center" }}>
                    <X size={14} />
                  </motion.button>
                </motion.div>
              )}
            ...
            {/* Filter Bar */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px", borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                {["All", "Images", "Videos", "Audio"].map((tab, i) => (
                  <motion.button key={tab} whileTap={{ scale: 0.95 }}
                    style={{ height: "28px", padding: "0 12px", borderRadius: "8px", border: `1px solid ${i === 0 ? C.border : "transparent"}`, background: i === 0 ? "rgba(255,255,255,0.09)" : "transparent", color: i === 0 ? C.text : C.textMuted, cursor: "pointer", fontSize: "12.5px", fontFamily: "inherit", transition: "all 0.14s ease" }}
                  >{tab}</motion.button>
                ))}
                <label style={{ display: "flex", alignItems: "center", gap: "6px", marginLeft: "6px", fontSize: "12.5px", color: C.textMuted, cursor: "pointer" }}>
              ...
              {generating ? (
                <div style={{ height: "100%", display: "grid", placeItems: "center" }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <p style={{ margin: 0, color: C.textMuted, fontSize: "13px" }}>{mode} · {ratio} · {outputCount} outputs</p>
                  </div>
                </div>
              ) : showRecent ? (
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
                    <span style={{ fontSize: "12px", color: C.textMuted, fontWeight: 500 }}>Recent Generations</span>
