...
real human skin, visible pores, fine micro-texture, subtle uneven pigmentation,
natural roughness, realistic facial shading, light falloff across skin,
slight natural oil variation on high points (nose, cheeks, forehead),
subtle under-eye depth, no smooth gradients

unretouched, no beauty filter, no airbrushed skin, no plastic or waxy texture,
no skin smoothing, no AI glow
`.trim();

const CAMERA_REALISM_BLOCK = `
shot on real camera, natural lens response,
subtle sensor grain, realistic dynamic range,
no digital smoothing, no artificial face enhancement
`.trim();

const LIGHT_SKIN_INTERACTION_BLOCK = `
lighting must interact with skin realistically,
directional light creates uneven illumination,
skin must show shadow falloff and highlight rolloff,
no flat lighting, no evenly lit face,
face brightness must follow light direction only
`.trim();

const MICRO_DETAIL_BLOCK = `
high frequency skin detail, micro-contrast on skin surface,
no soft blending, no gradient smoothing across face
`.trim();
