...
export const INSTANTID_MODEL =
  process.env.REPLICATE_INSTANTID_MODEL || "grandlineai/instant-id-photorealistic";
...
export function getModelForTask({ hasCharacter = false, attempt = 0 } = {}) {
  ...
  const instantIdEnabled = Boolean(process.env.REPLICATE_INSTANTID_MODEL);

  if (attempt <= 0 || !instantIdEnabled) {
    return {
      provider: "fal",
      model: DREAMO_MODEL,
