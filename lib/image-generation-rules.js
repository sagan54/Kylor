...
const BASE_SKIN_REALISM_BLOCK = [
  "SKIN REALISM:",
  "Natural clean healthy skin with visible pores, subtle tonal variation, realistic facial shading, slight natural under-eye depth, and believable texture.",
  "Keep skin photorealistic and clean without glamour retouching.",
  "No beauty filter, no airbrushed skin, no waxy skin, no plastic skin, no excessive skin smoothing.",
  "Do not add acne, pimples, damaged skin, or dirty skin unless explicitly requested.",
].join(" ");
...
export function buildSkinRealismBlock(scene = "", style = "") {
  const sceneTypes = detectSceneTypes(scene, style);
  const lines = [BASE_SKIN_REALISM_BLOCK];
