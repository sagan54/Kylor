...
const IDENTITY_PRESERVATION_BLOCK = [
  ...
  "Preserve facial structure, bone structure, skin tone, eye shape, nose, jawline, and exact likeness.",
  "Do not preserve the reference expression, mood, or facial muscle tension.",
...
const MICRO_DETAIL_BLOCK = `
subtle natural skin micro-detail, believable pores and fine facial texture,
realistic eyelash and eyebrow detail, natural lip texture,
authentic under-eye detail, realistic hair strand separation,
preserve fine facial detail without artificial sharpening
`.trim();
...
const EXPRESSION_ENFORCEMENT_BLOCK = [
  "CRITICAL EXPRESSION RULE:",
  "Facial expression MUST match the scene emotion exactly.",
  "Do not default to neutral or smiling expression.",
  "Expression must show visible muscle tension and emotional clarity.",
  "Mismatch between scene and expression is not allowed.",
].join(" ");
...
export function inferSceneExpression(promptText = "", style = "") {
  ...
  let guidance =
    "Expression must stay focused and scene-driven by default, with visible facial intention and no smile unless the scene clearly calls for it.";
  let muscleGuide =
    "Brows lightly engaged, eyes attentive, mouth neutral but not posed, jaw naturally set.";
  let weighted = "(focused expression, visible facial intent, no smile:1.3)";
  let negative =
    "happy expression, cheerful grin, big smile, playful smile, neutral expression, relaxed face, emotion mismatch";
  ...
  if (has(/victory|warm reunion|joy|joyful|happiness|happy|celebration|laughing|laugh|smile|smiling|wedding toast/)) {
    ...
    muscleGuide =
      "Brows softened, eyes warm, mouth naturally lifted or gently smiling, jaw relaxed.";
    weighted = "(subtle joyful expression, warm eyes, natural smile:1.35)";
    ...
    muscleGuide =
      "Brows lowered or drawn upward with sadness, eyes softened or wet, mouth heavy and restrained, jaw relaxed with emotional weight.";
    weighted = "(sad emotional face, visible grief, no smile:1.4)";
    negative = "happy expression, cheerful grin, playful smile, triumphant look, neutral expression, relaxed face";
    ...
    if (has(/survival|stand ground|fight back|determined|resist/)) {
      ...
      muscleGuide =
        "Brows raised and tightened, eyes alert and strained, mouth tense, jaw set under pressure.";
      weighted = "(fearful determined expression, survival tension, no smile:1.4)";
    } else {
      label = "fearful / shocked";
      guidance =
        "Expression should show fear, alarm, or shock appropriate to danger, not happiness.";
      muscleGuide =
        "Brows raised, eyes widened, mouth tense or slightly open, jaw tight with alarm.";
      weighted = "(fearful shocked expression, wide eyes, tense mouth:1.4)";
    }
    negative = "happy expression, relaxed smile, flirtatious look, playful grin, neutral expression, relaxed face";
  } else if (has(/fight|battle|attack|rage|combat|assault|gunfight|war|punch|kick|aggressive|boxing|mid-punch/)) {
    ...
    muscleGuide =
      "Brows furrowed, eyes intense, nostrils slightly flared, mouth tight or teeth clenched, jaw visibly tense.";
    weighted = "(intense angry expression, teeth clenched, combat tension:1.4)";
    negative = "happy expression, cheerful grin, relaxed smile, soft portrait smile, neutral expression, relaxed face";
  } else if (has(/angry|anger|furious|mad|enraged/)) {
    label = "angry / tense";
    guidance =
      "Expression should show anger clearly with no neutral fallback and no smile.";
    muscleGuide =
      "Brows drawn down, sharp gaze, nostrils slightly flared, mouth compressed or tense, jaw tight.";
    weighted = "(intense angry expression, sharp gaze, tight jaw:1.4)";
    negative = "happy expression, cheerful grin, relaxed smile, neutral expression, relaxed face";
  } else if (has(/fear|afraid|frightened|scared|terrified/)) {
    label = "fearful / tense";
    guidance =
      "Expression should clearly show fear with visible tension and no smile.";
    muscleGuide =
      "Brows raised, eyes wide, mouth tense, jaw tight or slightly dropped in fear.";
    weighted = "(fearful expression, wide eyes, tense mouth:1.4)";
    negative = "happy expression, relaxed smile, neutral expression, relaxed face";
    ...
    muscleGuide =
      "Brows engaged, eyes fixed and tense, mouth firm, jaw controlled and set.";
    weighted = "(intense focused expression, locked gaze, no smile:1.35)";
    negative = "happy expression, grin, playful look, casual smile, neutral expression, relaxed face";
    ...
    muscleGuide =
      "Brows tightened, eyes strained, mouth tense or partially open from discomfort, jaw under stress.";
    weighted = "(painful strained expression, fatigue, visible tension:1.4)";
    negative = "happy expression, relaxed grin, playful smile, neutral expression, relaxed face";
    ...
    muscleGuide =
      "Brows slightly drawn, eyes steady, mouth firm, jaw controlled with resolve.";
    weighted = "(determined heroic expression, steady gaze, no smile:1.35)";
    negative = "happy expression, silly grin, casual smile, relaxed face";
  } else if (has(/calm|quiet|stillness|peaceful|meditative|resting/)) {
    label = "calm / neutral";
    guidance =
      "Expression should stay calm, grounded, and softly neutral, without unnecessary happiness or dramatic tension.";
    muscleGuide =
      "Brows relaxed, eyes soft, mouth neutral, jaw loose and natural.";
    weighted = "(calm relaxed face, soft gaze, natural neutrality:1.25)";
...
export function buildMicroDetailBlock() {
  return MICRO_DETAIL_BLOCK;
}
...
export function buildNegativeRealismBlock() {
  return NEGATIVE_REALISM_BLOCK;
}

export function buildExpressionBlock(scene = "", style = "") {
  const inferred = inferSceneExpression(scene, style);
  return [
    "EXPRESSION CONTROL (HIGH PRIORITY):",
    inferred.guidance,
    `Facial muscle behavior: ${inferred.muscleGuide}`,
    `Target expression: ${inferred.label}.`,
    `Emphasis: ${inferred.weighted}.`,
    EXPRESSION_ENFORCEMENT_BLOCK,
  ].join(" ");
}
...
export function buildExpressionEnforcementBlock() {
  return EXPRESSION_ENFORCEMENT_BLOCK;
}
...
export function buildPrompt({
  ...
  const characterBlock = character
    ? [
        ...
        "Do not preserve expression, mood, or facial muscle tension from the reference.",
      ].join(" ")
    : "";
